...
const DiscountCarousel = ({ data = [], autoplayDelay = 4000 }) => {
  const dispatch = useDispatch();
  const theme = useTheme();

  // Responsive itemsPerSlide
  const isSmall = useMediaQuery(theme.breakpoints.down("sm")); // 1 card
  const isMedium = useMediaQuery(theme.breakpoints.between("sm", "md")); // 2 cards
  const itemsPerSlide = isSmall ? 1 : isMedium ? 2 : 3;
